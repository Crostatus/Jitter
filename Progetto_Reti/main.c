...
static const int a = 999, b = 9758;    /* for hash function*/
...
long int get_elapsed_time(struct timespec start, struct timespec end){
  long int seconds_in_nanos_start =  start.tv_sec * 1000000000;
  long int seconds_in_nanos_end = end.tv_sec * 1000000000;

    //long int nseconds_in_millis = (end.tv_nsec - start.tv_nsec) / 1000000;
    return (seconds_in_nanos_end + end.tv_nsec)-(seconds_in_nanos_start + start.tv_nsec);
}

struct timespec get_timespec_from_nanos(long int tempo){
  long int residuo;
  struct timespec t1;
  t1.tv_sec = 0;
  residuo = tempo;
  while(residuo - 1000000000 > 0){
    t1.tv_sec++;
    residuo = residuo - 1000000000;
  }
  t1.tv_nsec = residuo;
  printf("Tempo: %ld Secondi_ %ld Nanosecondi: %ld\n", tempo, t1.tv_sec, t1.tv_nsec);
  return t1;
}
