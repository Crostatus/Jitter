...
int main(int argc, char *argv[]){
    int sd; // No data/payload just datagram
    ...
    din.sin_port = htons(9999);
    // IP addresses
    sin.sin_addr.s_addr = inet_addr(argv[1]); 
    din.sin_addr.s_addr = inet_addr(argv[2]); //"23.196.169.157" Indirizzo Repubblica
    ...
    for(count = 1; count <=20; count++){
        if(sendto(sd, buffer, sizeof(buffer), 0, (struct sockaddr *)&din, sizeof(din)) < 0){
            // Verify
            perror("sendto() error");
            exit(EXIT_FAILURE);
        }else{
